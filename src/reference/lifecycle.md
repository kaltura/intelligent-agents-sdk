...
**On this page:** [Rule shape](#rule-shape) · [The four action types](#the-four-action-types) · [Scoping a rule to one agent](#scoping-a-rule-to-one-agent) · [Every session already gets a SUMMARY, for free](#every-session-already-gets-a-summary-for-free) · [Discovery and dry-run testing](#discovery-and-dry-run-testing) · [Full CRUD + discovery method table](#full-crud--discovery-method-table) · [Related docs](#related-docs)
...
This page is the field-by-field reference. New to Lifecycle? [Lifecycle Recipes](/guides/lifecycle-recipes/) is a hands-on walkthrough of the two action types you actually create, with a runnable example — it links back here instead of repeating this reference.
...
| Doc | What it adds |
|---|---|
| [Lifecycle Recipes](/guides/lifecycle-recipes/) | Hands-on walkthrough of `triggerInsight` + `sendInsightEmail` chained together, common pitfalls, and a runnable example |
