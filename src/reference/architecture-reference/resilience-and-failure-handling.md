...
| All agent slots busy | `throwToNoAgent` | Availability queue + poll (see [Scale & Sticky Sessions](/reference/architecture-reference/scale-and-sticky-sessions/#scale--sticky-sessions)) |
...
- TURN relay for connectivity behind hostile NATs (STV forces relay; ASR relays in practice regardless of policy — see [Endpoints & Credentials](/reference/architecture-reference/connection-and-handshake/#endpoints--credentials)).
