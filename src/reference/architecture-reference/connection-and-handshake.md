...
This 30s figure is unrelated to the platform's built-in client's own internal per-attempt connect timeout used by the capacity queue — see [Capacity & the queue](/reference/architecture-reference/scale-and-sticky-sessions/#capacity--the-queue-throwtonoagent--throwtoexceededtier) for that separate constant and how the two don't compose.
