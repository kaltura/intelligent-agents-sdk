...
- **`followups-tool`, `flashcards-tool`, and `show-link-tool` have been exercised against real backend output** — including the `followups-tool`/`show-link-tool` boundary flush (a different runtime arriving mid-stream closes the prior widget correctly), end to end through `SegmentAssembler` → `ExperienceRenderer` → `mountWidget` → click → `KavaAnalytics.buttonClicked()` (see [GenUI · Widget Analytics](/reference/genui/analytics/)). The other six runtimes are implemented from the schema and have not yet been observed from the backend.
