...
**Note:** `force_experience` is not an intellect field — it's a per-call option on `converse()` (values `"markdown"`, `"summarization"`, `"flashcards"`, `"avatar_only"`; a hint, not a guarantee). See [Converse](/reference/api/operate/#converse).
