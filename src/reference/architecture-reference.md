...
`type` values: `think`, `text`, `unisphere-tool`, `tool`, `tool_response`, `avatar`, `error`, `share`, `thread` — the same set as `/assistant/converse` (the live runtime wraps the same brain stream). The first `agent_raw_text` on the live socket additionally carries an **`init_response`** delta (`openingPhrase`/`threadId`/`messageId`) — that one is a WebSocket-only frame from the Genie brain backend's websocket handler, not an HTTP-converse segment. The `type` is the LLM's code-fence tag (open-ended) for content blocks, plus the fixed control types `think`/`tool`/`tool_response`/`error`; see [Wire Protocol §4e](/reference/wire-protocol/#4e-agent_raw_textdelta--the-brain-stream-parsed) for the complete catalog, including the socket-only `interruption`/`user-interruption` control types (no HTTP-converse equivalent — see [API § Operate](/reference/api/operate/#converse)).
...
| `conversations.js` | `Conversations` (`stream`/`send`, `assertRequestVars`), `Threads`/`Messages`/`Feedback`/`Followups`, `Knowledge` (`addRecord` + `knowledge_ids` linkage, ungated; `list`/`listCategoryEntries`, `uploadDocument`, `createCategory`/`findOrCreateCategory`, `addSource`/`removeSource`, `corpusStatus`, `getLinkage`, `setEnabled`, `search`, `isIndexed`, `entryStatus`). | Genie `assistant/converse` (converse); Genie `v1/knowledge/add` + intellect `knowledge_ids` (ungated); OVP `category/*`+upload (containers) |
...
| Field | What it does |
|---|---|
| `threadId` | Resume an existing conversation thread |
| `sse` | Choose SSE framing over NDJSON for the stream |
| `model_type` | Model-speed hint forwarded to the brain |
| `force_experience` | Experience hint forwarded to the brain |
| `request_vars` | Per-turn client variables — `assertRequestVars` rejects reserved keys and non-scalar values before the wire |
| `capabilities` | Per-message `{name: state}` override, validated client-side. The server-side **DISABLED veto still wins**: a stored/env-disabled capability cannot be turned on per message (e.g. `{capabilities:{use_web_search:'on'}}` is honored only if `use_web_search` is not disabled by a stored layer) |
| `recoverFromSpiral` | On `conversations.send`/`converseOnce`: when the first attempt comes back `spiralStopped:true` with empty text, send one same-thread nudge retry (`SPIRAL_RECOVERY_PREFIX`) — see `stream.js`'s `collectConverse` entry above for what it's recovering from |
...
### Routing rule: which door a field writes through

This is the load-bearing design rule for the management layer — **a write goes to exactly one public door, or to no door at all**:

- **Intellect DTO** → Genie `v1/intellect/*`. Carries `prompts`, `base_directive`, `glossary`, `capabilities`, `tools`, `secrets`, `user_properties_forms`, `allow_client_variables`, `knowledge_ids`, `name`/`description`/`tags`/`status`. Writable with a **partner admin KS** today. **Knowledge linkage rides this door**: first call `POST /v1/knowledge/add` on Genie (LIVE — returns an `{id,...}` record), then pass the returned id as `knowledge_ids` in the intellect create/update DTO — linkage + `use_knowledge_base:'on'` persist with no separate gate or 403. It is a `model_fields_set` PATCH (omitted TOP-LEVEL fields are preserved) — but `capabilities`/`secrets` are **full-replace sub-dicts**, so the SDK read-merge-writes them (via `mergeCapabilityWrite` / the secrets mask-and-keep guard); `IntellectConfig.patch` is the one place that logic lives.
- **Brain-model and rate-limit fields have no public write door at all.** `agent_llm`/`agent_fast_llm`/`agent_avatar_llm`/rate limits/`run_quota_check`/`web_search_config` exist on the backend intellect record, but neither `v1/intellect/*` nor any other public route reads or writes them — they're set by internal tooling only. `IntellectConfig.describe(configId, ks)` surfaces their current values (where readable) under its `readOnly` map, purely informational (see [§ Brain-Model & Rate-Limit Fields](/reference/api/build/#brain-model--rate-limit-fields-not-in-the-public-api)). An earlier `partner-config/update`/`get` route family that once carried these fields (gated, 403ing on every deployment, with no working backend route) has been removed from the SDK's public surface entirely — there is no gated door to probe anymore, only no door.

When designing a new field setter, decide its door by which DTO genuinely accepts it (the `IntellectConfig` `EDITABLE_FIELDS` vs `READ_ONLY_FIELDS` constants encode this) — a field with no accepting DTO belongs in `READ_ONLY_FIELDS`, surfaced only through `describe()`'s informational `readOnly` map, never given a setter that would silently no-op.

### Honest limits surfaced by the SDK

[SDK Reference's "Honest limits"](/reference/sdk-reference/#honest-limits) covers the brain-model/rate-limit no-public-door limit, no-verbatim-speech, and the `force_experience`/`model_type` hint caveats — read that first. The rest are architecture-level limits not covered there:
...
**What `KalturaAvatarSession` implements** (custom clients bypassing the SDK must rebuild these): ICE restart (`_recoverMedia`), socket-transport recovery, a repeating brain-stall watchdog + `brainStalled` event, a two-tier tool-call-spiral circuit breaker (`toolSpiralDetected` soft, `toolSpiralRecovering` + cold reconnect hard), granular device error codes (`NotAllowed`/`NotFound`/`NotReadable`), and `online`/`offline`/`visibilitychange` handling.
...
- **Fresh socket, not re-join.** The control socket is still live at this point (unlike a genuine transport drop), and the server's `join` handler silently no-ops a re-join on a live socket (reproduced live as `JoinRoomTimeout`). So `_coldReconnect()` detects this case (`state !== 'reconnecting'` at entry) and opens a genuinely new socket before re-`join`-ing. Only the genuine-transport-drop path (`state === 'reconnecting'`) safely reuses the existing socket — there the server has already discarded the session.
- **Re-arm on successful reconnect.** A spiral by definition never produces spoken/GenUI content, so a perceivable-output reset can't fire while one is running. The guard therefore re-arms on a successful cold reconnect; without that, a second spiral in the same session would find the guard latched and hang.

A cold reconnect restores connectivity and brain memory (`threadId`) but abandons the turn that triggered it. With `recoverFromSpiral` (default `true`), the SDK auto-resends that turn's tracked text once (from `speak()` or ASR's `userTranscription`), prefixed with `SPIRAL_RECOVERY_PREFIX`, and emits `spiralRecovered {text}`; `false` leaves it to the app via `lastTurnText`. All three thresholds (`brainStallMs`, `toolSpiralLimit`, `hardToolSpiralLimit`) are configurable at construction; `0` disables any of them. The avatar runtime client (the non-SDK reference implementation) has no such breaker. Author-side mitigation (a tool-call budget in the system prompt) is covered in [Client-Side Commands](/guides/client-commands/)'s "Tool spirals starve the voice".
