...
**On this page:** [Reporting a vulnerability](#reporting-a-vulnerability) · [Table of contents](#table-of-contents) · [AI-application controls (OWASP LLM/Agentic; HIPAA technical safeguards)](#ai-application-controls-owasp-llmagentic-hipaa-technical-safeguards) · [KS (Kaltura Session) guidance for agents (AC-3 / AC-6 / IA-2)](#ks-kaltura-session-guidance-for-agents-ac-3--ac-6--ia-2) · [Token lifecycle (RFC 9700 OAuth 2.0 Security BCP; NIST AC family)](#token-lifecycle-rfc-9700-oauth-20-security-bcp-nist-ac-family) · [Audit logging (NIST AU-2 / AU-3 / AU-12; OWASP Logging; SOC 2 CC7)](#audit-logging-nist-au-2--au-3--au-12-owasp-logging-soc-2-cc7) · [Transport security (NIST SC-8; OWASP WSS/TLS)](#transport-security-nist-sc-8-owasp-wsstls) · [Browser hardening (OWASP ASVS / WebSocket CS)](#browser-hardening-owasp-asvs--websocket-cs) · [Isolation & multi-tenancy (NIST SC-4 / AC-6(4))](#isolation--multi-tenancy-nist-sc-4--ac-64) · [Supply-chain integrity (SLSA / OpenSSF / EO 14028)](#supply-chain-integrity-slsa--openssf--eo-14028) · [Shared-responsibility control matrix (NIST 800-53)](#shared-responsibility-control-matrix-nist-800-53) · [FIPS mode (how-to)](#fips-mode-how-to) · [Data residency (SC-7)](#data-residency-sc-7) · [Framework crosswalks](#framework-crosswalks)
...
- **Avatar/deepfake.** On by default: disclosure-before-speech with `synthetic`/`provenance` data, fired before the avatar's first words on every connect. `getDisclosure()` is queryable any time. Opt-in: `requireDisclosureAck` (also a biometric-consent gate) and an optional `consentRef` on voice/visual cloning.
...
| `session.complete` | The session-completion signal (`POST /thread/session_completed`) fires on disconnect, tab-close, backgrounding, or bfcache freeze. It reuses the session's own conversation KS and mints no new credential. `action` carries the trigger reason (`disconnect`, `pagehide`, `hidden_grace`, `suppressed:peers=N`, …). See [System Internals Reference § Session-completion signal](/reference/architecture-reference/resilience-and-failure-handling/#session-completion-signal-session_completed-telling-the-backend-a-conversation-is-truly-over) |
...
- CSP: the SDK uses no `eval`/`new Function`. A working policy sets:
  - `connect-src` to your live-session control-plane host (`conversationManagerUrl`), the WHEP video-egress host (`srsBaseUrl`), and your TURN host
  - `media-src blob:`
  - `script-src` to your injected socket.io origin, pinned with SRI (see the README's "Injecting socket.io securely")

  Also set `frame-ancestors` on the embedding page. A mic-capable widget needs anti-clickjacking headers.
