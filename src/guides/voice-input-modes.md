...
Chat mode is a full text-only transport, with the same [brain](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#architecture), same tools, and same thread as the voice modes. It's covered in [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#text-only-chat-kalturachatsession), including mid-conversation switching between avatar and chat via `KalturaAgentSession` ([switching section below](#switching-between-avatar-and-chat-mid-conversation)).

For the wire mechanics of `startTapToTalk()`/`endTapToTalk()`, see [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#tap-to-talk-push-to-talk-voice). For the exact socket events, see [Wire Protocol](/reference/wire-protocol/) (`tapToTalkStart`/`tapToTalkEnd`, `isTapToTalk`).

Text input is not voice-mode-exclusive: an avatar session accepts typed turns too (`session.speak(text)` — see [Client-Side Commands](/guides/client-commands/) and README). Offering a text box *alongside* either voice mode is how the SDK satisfies EN 301 549 §6.2.1.2 (concurrent voice and text) — see [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#accessibility-wcag-22-aa--captions--ai-disclosure-gate).
...
3. Wire click-to-toggle calling `session.startTapToTalk()`/`session.endTapToTalk()`, updating `aria-pressed` and an icon/level indicator on `tapToTalkStarted`/`tapToTalkEnded` — see the conditional-UI example in [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#tap-to-talk-push-to-talk-voice).
...
| Doc | What it adds |
|-----|---------------|
| [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#tap-to-talk-push-to-talk-voice) | The SDK API: `startTapToTalk()`/`endTapToTalk()`, the `capability_disabled` gate, `tapToTalkActive`/`capabilities.tapToTalk` |
| [Wire Protocol](/reference/wire-protocol/) | The exact `tapToTalkStart`/`tapToTalkEnd` socket events and the finding behind the mixed-mode gate |
...
| [README.md](https://github.com/kaltura/intelligent-agents-sdk/blob/main/README.md#text-only-chat-kalturachatsession) | `KalturaChatSession` (text-only transport) and `KalturaAgentSession` (mode switching) — full API |
