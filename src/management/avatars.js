 ...
 * tools/agentic.mjs avatar-* + API-REFERENCE §2.4 / Management Operations.
 */
 ...
 * avatar DTO is strict-rejecting (verified: `avatar/create` 400s with
...
export class Avatars {
   ...
   * verified `bad_request` (`body.detail: ["property adminTags should not
   ...
   * omitted) has historically triggered a server-side crash surfaced to end
   * users right after `showAgent`; still avoid a falsy value defensively. If
   * your intellect drives its own dynamic opening (e.g. based on the viewer's
   * prior interactions) and you want no scripted greeting, pass the SSML
   * silence tag `'<blank>'` — non-empty, so it
   * stays on the safe path, and silent, so the TTS speaks nothing for it.
   *
   ...
   * the body are PRESERVED server-side (verified — sending `{id, openingPhrase}`
   ...
   * `adminTags` with a verified `bad_request`, but the raw server reply is only a
