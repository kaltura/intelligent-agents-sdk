...
**In the browser, let the agent speak first.** Pass `kickoff` to the session and the SDK sends that text as the first turn, once, as soon as the server accepts input. With the silent opening the quickstart set, the agent's own greeting starts about two seconds after `connect()` resolves and the user can interrupt it. Guide: [docs/START-THE-CONVERSATION.md](docs/START-THE-CONVERSATION.md).
