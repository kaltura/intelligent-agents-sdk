...
That runs the full suite (unit + integration + e2e + evals) against `test/fakes/fetch.js`, `test/fakes/socket.js`, and `test/fakes/rtc.js`. These are deterministic stand-ins for the real HTTP/socket/WebRTC backends. It takes about 105 seconds on a typical machine.
