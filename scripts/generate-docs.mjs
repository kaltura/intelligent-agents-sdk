...
import { manifest, findBySource, findByTarget, basenameIndex } from './docs-manifest.mjs';
...
function generateNav(siteDir) {
  ...
  for (const entry of manifest) {
    ...
    groups.get(entry.group).push({ title: entry.navTitle, url: entry.url, children: buildNavChildren(entry.target) });
  }
  const sections = order.filter((group) => groups.has(group)).map((group) => ({ group, pages: groups.get(group) }));
  const lines = [
    ...
    `module.exports = ${JSON.stringify(sections, null, 2)};`,
  ];
  writeFileSync(resolve(siteDir, 'src', '_data', 'nav.js'), lines.join('\n') + '\n', 'utf8');
}

// A page hidden from the top-level sidebar list (no `group`) carries a `hub`
// field pointing at its parent's `target`. This collects, for one hub, every
// entry that names it — recursing so a hub that's itself hidden (e.g.
// api/build.md, hidden under api-reference.md but a hub for its own 5
// sub-pages) nests correctly two levels deep.
function buildNavChildren(target) {
  return manifest
    .filter((e) => e.hub === target)
    .map((e) => ({
      title: (e.navTitle || e.title).split(' · ').pop(),
      url: e.url,
      children: buildNavChildren(e.target),
    }));
}

// Every page hidden from nav.js also needs a breadcrumb trail walking its
// `hub` chain back up to the sidebar-visible ancestor, consumed by base.njk's
// breadcrumb UI. Some hubs (e.g. api/build.md) are themselves hidden and have
// their own `hub`, hence the loop instead of a single lookup.
function generateHubs(siteDir) {
  const trails = {};
  for (const entry of manifest) {
    if (!entry.hub) continue;
    const trail = [];
    let hubTarget = entry.hub;
    while (hubTarget) {
      const hubEntry = findByTarget(hubTarget);
      if (!hubEntry) {
        throw new Error(`generate-docs: hub '${hubTarget}' referenced by '${entry.target}' does not resolve to a manifest entry`);
      }
      trail.unshift({ title: hubEntry.navTitle || hubEntry.title, url: hubEntry.url });
      hubTarget = hubEntry.hub;
    }
  ...
  const lines = [
    ...
    `module.exports = ${JSON.stringify(trails, null, 2)};`,
  ];
  writeFileSync(resolve(siteDir, 'src', '_data', 'hubTrails.js'), lines.join('\n') + '\n', 'utf8');
}
...
async function main() {
  ...
  generateNav(siteDir);
  generateHubs(siteDir);
  console.log(`generate-docs: wrote ${count} page(s) + nav.js + hubTrails.js from ${sdkDir}`);
}
