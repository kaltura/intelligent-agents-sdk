...
// Every page hidden from nav.js (no `group`) carries a `hub` field pointing at
// its parent's `target`. Walk that chain up to the sidebar-visible ancestor to
// build a breadcrumb trail, and collect siblings (other entries with the same
// immediate hub) for the "other pages here" list — both consumed by base.njk's
...
function generateHubs(siteDir) {
  ...
  for (const entry of manifest) {
    ...
    // Sibling titles encode full hub lineage ("Agent Components · Preview a
    // Prompt") for standalone display (page <title>, etc); next to a
    // breadcrumb that already names the hub, only the last segment is useful.
    const siblings = manifest
      .filter((e) => e.hub === entry.hub && e.target !== entry.target)
      .map((e) => ({ title: (e.navTitle || e.title).split(' · ').pop(), url: e.url }));
    trails[entry.url] = { trail, siblings };
  }
  const lines = [
