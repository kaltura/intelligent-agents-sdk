...
You configure it once, at [intellect](../README.md#architecture) creation or update, as a list of "stages":
...
- Editable only through the SDK's `intellect/*` surface. `user_properties_forms` is authored once at provisioning time via the SDK against the [brain](../README.md#architecture)'s API, not tunable per-session through any other surface.
...
A worked pattern: keep the submitted values in browser memory for the current session, but do the durable write via your own model-called, server-side `api` tool that posts to whatever external system you point it at, rather than `setFormLeadInfo`. See [EXTERNAL-API-INTEGRATIONS.md](EXTERNAL-API-INTEGRATIONS.md) for the general pattern this specializes.
