...
`session.submitStructuredDataForm(info)` (`src/experience/session.js`) is a fire-and-forget socket emit — `this._socket.emit('setFormLeadInfo', sanitizeJson(info))` — with no acknowledgment payload, and no endpoint on the brain/management API reads it back as structured `{key: value}` data. The conversation transcript is persisted by the brain (as a message record) and exposed read-only via `POST /thread/get_transcripts` — exactly what the management SDK's `threads.transcript(threadId, ks)` wraps. That reconstructs a plain-text transcript from `USER`-type message rows; it does **not** carry the structured form field values, only what the viewer said/typed and the model's replies — a paraphrase, not the raw object.
