...
| `unisphere-tool` | control (brain backend's response-formatter, structured-experience emission) | structured-experience block. First segment carries `metadata:{widgetName, runtimeName}`; known runtimes `followups-tool`, `flashcards-tool`. See [§7](client-configuration.md#7-clientconfiguration-fields-per-session-agent-config). |
