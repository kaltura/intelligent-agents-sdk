...
If your use case is specifically getting the *viewer's own submitted data* (from a `user_properties_forms` prompt) onto external infrastructure, read [STRUCTURED-DATA-FORMS.md](STRUCTURED-DATA-FORMS.md) first. It explains why `session.submitStructuredDataForm()` alone does **not** get you durable, retrievable data with this toolkit's credentials. Everything here is the alternative: a **tool call** the model makes directly, landing on infrastructure you control.
