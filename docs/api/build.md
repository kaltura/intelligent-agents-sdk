...
- `{ actionType: 'triggerDataToCollectInsight' }` — system preset only, and that preset is currently disabled account-wide. Would extract one insight per configured lead-capture field (`intellectConfig.user_properties_forms`, see [§ Configure an Intellect](#configure-an-intellect)) if ever enabled. Not something to create yourself.
