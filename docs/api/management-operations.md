...
| Feedback report (CSV) | `POST /feedback/report` | `{"filter":{"objectType":"GenieListFeedbackFilter"}}` — reserved on the backend for a future release: currently always replies an empty body, for every partner and filter. The SDK returns `null` in that case rather than throwing. Use `mgmt.feedback.list()` or `mgmt.messages.report()` for feedback data today. |
