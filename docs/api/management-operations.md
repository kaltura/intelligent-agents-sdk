...
| Feedback report (CSV) | `POST /feedback/report` | `{"filter":{"objectType":"GenieListFeedbackFilter"}}` — ⚠️ SENSITIVE: contains end-user ids/names + verbatim question/feedback text. Replies an empty body when the partner has none; the SDK returns `null` in that case rather than throwing. |
