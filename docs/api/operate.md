...
| `sys__context_id` | The category/entry id the current context (and its knowledge base, if any) is scoped to | Set via `contextId` on the live socket `join` payload — see [connection-and-handshake.md § The `join` payload](../architecture-reference/connection-and-handshake.md#the-join-payload-step-2--this-carries-the-agentbrain-config). Empty when no context was set at join. |
| `sys__context_type` | The type of that context (e.g. an `entry` vs. a `category`) | Set via `contextType` alongside `sys__context_id`; empty when no context was set at join. |
