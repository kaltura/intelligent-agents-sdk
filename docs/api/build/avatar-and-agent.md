...
| A curated template + whatever it's missing | `templateId` + whichever of `face`/`background`/`visual` the template doesn't already supply | Fastest good-looking result — see below. `templateId` alone is a domain failure unless the template already has its own `visual`. |
