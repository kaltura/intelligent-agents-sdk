...
| `opening_phrase` | Jinja2 template over `request_vars` (e.g. `"Hello {{ user_name }}!"`) the backend renders and speaks when an avatar session starts. Overrides the avatar's own `openingPhrase`; the rendered text comes back in the init response and is stored on the thread as an `opening` message. `null` clears it. The opening turn cannot be interrupted: pass `SILENT_OPENING` for no spoken opening and send the first turn from the browser with `kickoff` ([START-THE-CONVERSATION.md](../../START-THE-CONVERSATION.md)). Set via `intellectConfig.setOpeningPhrase` |
