...
| `opening_phrase` | The line spoken as the first turn of every avatar session. The intellect owns it: `provision({ openingPhrase })` writes it here and leaves the avatar's `openingPhrase` unset. A Jinja2 template rendered once per session over `requestVars` (needs `allow_client_variables`) and `sys__*`, e.g. `{% if user_name %}Hello {{ user_name }}!{% else %}Hello!{% endif %}`; the rendered text is stored on the thread as an `opening` message. `null` clears it. The opening turn cannot be interrupted: pass `SILENT_OPENING` for no spoken opening and send the first turn from the browser with `kickoff`. Guide: [START-THE-CONVERSATION.md § Personalize the opening](../../START-THE-CONVERSATION.md#personalize-the-opening). Set via `intellectConfig.setOpeningPhrase` |
