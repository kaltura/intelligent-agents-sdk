...
This page is the field-by-field reference. New to Lifecycle? [`recipes.md`](recipes.md) is a hands-on walkthrough of the action type you actually create, with a runnable example — it links back here instead of repeating this reference.
...
The backend recognizes four `actionType` values. Only one is currently safe for you to create; the other three only exist to power system preset rules or have been retired.
...
| `triggerInsight` | Nobody — retired | Rejected client-side with a `bad_request` before any network call | No longer supported. Use `sendInsightEmail` instead. |
...
A human gets notified the moment a conversation's analysis lands, with zero app-side polling.

---

## Scoping a rule to one agent

`eventConditions` can only filter on fields [`describeFields`](#discovery-and-dry-run-testing) actually reports. For `thread`/`analysis_updated` today that's `object.agent_id`, `object.thread_id`, `object.user_id`, and `changed_keys` (which insight keys were updated), **not** an insight's computed value — there is no field to filter on an insight's own extracted value, since that value only exists as the *output* of an insight-extraction action, not an input `eventConditions` can inspect.
...
A system-seeded rule, `preset__summary_on_session_ended`, runs automatically on every `session_ended` event, for every agent, with no opt-out and no rule of your own required.
...
None of this pushes the conversation transcript through a rule you write — the preset sends the backend's insight service a `threadId`; that service fetches the transcript itself.
  ...
  matchedRules: [
    {
      rules: [
        ...
        { id: 'rule-you-created', action: { actionType: 'sendInsightEmail', recipients: ['<kaltura-user-id>'] } },
      ],
      isGrouped: true,
...
| Doc | What it adds |
|---|---|
| [`recipes.md`](recipes.md) | Hands-on walkthrough of `sendInsightEmail`, common pitfalls, and a runnable example |
