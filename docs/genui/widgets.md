...
- For the full picture — configuration, why the agent treats a configured stage as mandatory, where `setFormLeadInfo` actually persists server-side, and how to deliver the collected data somewhere durable — see [../STRUCTURED-DATA-FORMS.md](../STRUCTURED-DATA-FORMS.md) and [../EXTERNAL-API-INTEGRATIONS.md](../EXTERNAL-API-INTEGRATIONS.md).
