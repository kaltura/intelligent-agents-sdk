...
<script type="module">
  ...
  import { KalturaAvatarSession } from 'https://cdn.jsdelivr.net/gh/kaltura/intelligent-agents-sdk@v1.1.0/src/experience/index.js';
  // ... same API as the local examples — see examples/browser-experience.html
</script>
```

Pin the tag (`@v1.1.0`, or whatever release you want) for anything you ship — jsDelivr caches a
...
- **KS types never mix.** `disableentitlement` = management (server only). `geniegpcid:<configId>` = conversation (entitlement ON). Which kind a browser session carries is your call at mint time — see [SECURITY.md](SECURITY.md#ks-kaltura-session-guidance-for-agents-ac-3--ac-6--ia-2).
